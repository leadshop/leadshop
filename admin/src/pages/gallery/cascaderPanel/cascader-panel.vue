    ...
    <div :class="[
      ...
      border && 'is-bordered'
        ...
        <div v-for="(menu, index) in menus" class="el-cascader-item">
            <el-autocomplete size="small" value-key="label" @keydown.enter.native.stop="handleEnter(searchValue[index],index)" :placeholder="`输入${getlevel(index)}分组名称搜索`" v-model="searchValue[index]" :fetch-suggestions="(queryString, cb)=>{
